...
# 参数：width height --（750 589）
#    模拟器顶部像素点：37
#    模拟器底部像素点：52
# box = [ 左 上 右 下 ]
# windowData = [ 模拟器边界左 上 右 下 总宽 总高 ]


# =================截图比对区域=================#
IMAGE_PATH = 'C:/Users/shliz/Desktop/Auto_GirlsFrontline/auto12_4e_1/initial_IMG/'  # 读取截图的路径
...
COMBAT_FINISH_IMAGE_BOX = [0.485, 0.47, 0.53, 0.51]  # 战役完成判断区域
...
RETURN_COMBAT_IMAGE_BOX = [0.75, 0.66, 0.90, 0.69]  # 回到作战界面判断区域
AMMO_IMAGE_BOX = [0.1252, 0.65, 0.2528, 0.711]  # 炸狗人形弹药狗粮判断区域
ECHELON_FORM_IMAGE_BOX = [0.89, 0.77, 1.0, 0.835]  # 梯队编成判断区域
...
CHAPTER_12_CLICK_BOX = [0.18, 0.65, 0.22, 0.70]  # 选择第12章
...
# 队伍放置点
AIRPORT_CLICK_BOX = [0.45, 0.71, 0.46, 0.73]  # 重型机场
COMMAND_CLICK_BOX = [0.40, 0.31, 0.41, 0.33]  # 指挥部

# 放置队伍
TEAM_SHIFT_CLICK_BOX = [0.42, 0.20, 0.48, 0.24]  # 切换成普通梯队
TEAM_SET_CLICK_BOX = [0.85, 0.75, 0.92, 0.78]  # 放置梯队

# 炸狗人形替换（HK416与M4 SOPMODⅡ互唤）
TEAM_FORM_BOX = [0.16, 0.748, 0.23, 0.765]  # 点击队伍编成
EXCHANGE_STEP1_CLICK_BOX = [0.16, 0.41, 0.20, 0.46]  # 点击一号位
EXCHANGE_STEP2_CLICK_BOX = [0.19, 0.27, 0.26, 0.34]  # 更换人形
EXCHANGE_STEP3_CLICK_BOX = [0.07, 0.097, 0.1, 0.13]  # 退出阵型编成

# 扛伤位（第五个）修复
REPAIR_INTERVAL = 5  # 隔多少轮修一次
...
# 开始作战
START_COMBAT_CLICK_BOX = [0.85, 0.83, 0.92, 0.87]  # 点击开始作战
...
# 切换人形
EXCHANGE_HUMANOID_EVACUATE_BOX = [0.75, 0.72, 0.82, 0.74]  # 撤离已补给人形
EXCHANGE_HUMANOID_POINT1_BOX = [0.54, 0.62, 0.62, 0.64]  # 确定撤离
EXCHANGE_HUMANOID_POINT2_BOX = [0.23, 0.09, 0.26, 0.12]  # 终止作战
EXCHANGE_HUMANOID_POINT3_BOX = [0.37, 0.61, 0.40, 0.64]  # 重新作战

# 计划模式
PLAN_MODE_CLICK_BOX = [0.04, 0.77, 0.10, 0.79]  # 点击计划模式
PLAN_POINT1_CLICK_BOX = [0.53, 0.485, 0.54, 0.495]  # 点击计划点1
PLAN_POINT2_CLICK_BOX = [0.577, 0.485, 0.585, 0.495]  # 点击计划点2
PLAN_START_CLICK_BOX = [0.88, 0.82, 0.98, 0.85]  # 点击执行计划

# 战役结算
ROUND_END_CLICK_BOX = [0.89, 0.84, 0.97, 0.87]  # 结束回合
...
RETIRE_CLICK_BOX = [0.84, 0.78, 0.90, 0.81]  # 点击拆解
...
def mouseClick(box, minTime, maxTime, exact_x=0, exact_y=0, flag=False):
    ...
    # android adb
    if flag:
        os.system('adb shell input tap {} {}'.format(clickX, clickY))
    # win32api
    else:
...
# 在选择梯队界面
def isNoAmmo():
    initImage = cv2.imread(IMAGE_PATH + "dogfood_ammo.png")
    capImage = getImage(AMMO_IMAGE_BOX)
    capImage = cv2.cvtColor(np.asarray(capImage), cv2.COLOR_RGB2BGR)
    return imageCompare(initImage, capImage)
...
# 在阵型编成界面
def isEchelonFormation():
    initImage = cv2.imread(IMAGE_PATH + "echelon_formation.png")
    capImage = getImage(ECHELON_FORM_IMAGE_BOX)
    capImage = cv2.cvtColor(np.asarray(capImage), cv2.COLOR_RGB2BGR)
    return imageCompare(initImage, capImage)
...
def combatMenuTo12_4e():
    ...
    mouseDrag(CHAPTER_DRAG_BOX, 0, -1, 2, 400, 0.001, 1)
...
def start12_4e():
    ...
    mouseClick(EPISODE_4_CLICK_BOX, 1, 2)
...
def setTeam():
    ...
    time.sleep(0.2)
    checkCount = 0
    while not isNoAmmo() and checkCount < 20:
        time.sleep(0.4)
        checkCount += 1
    if checkCount >= 20:
        return False
    time.sleep(0.2)
    print("ACTION：更换榴弹人形")
    mouseClick(TEAM_FORM_BOX, 1, 2)  # 点击编成
    checkCount = 0
    while not isEchelonFormation() and checkCount < 20:
        time.sleep(0.4)
        checkCount += 1
    if checkCount >= 20:
        return False
    mouseClick(EXCHANGE_STEP1_CLICK_BOX, 1, 2)  # 点击一号位
    mouseClick(EXCHANGE_STEP2_CLICK_BOX, 1, 2)  # 更换人形
    mouseClick(EXCHANGE_STEP3_CLICK_BOX, 6, 7)  # 退出阵型编成
    adjustMap(True)  # 少量缩放
    time.sleep(1)
    mouseClick(COMMAND_CLICK_BOX, 1, 1)  # 点击指挥部
    checkCount = 0
    while not isSetTeam() and checkCount < 20:
        time.sleep(0.4)
        checkCount += 1
...
def supplyTeam():
    print("ACTION: 补给打捞队")
    mouseClick(COMMAND_CLICK_BOX, 1.5, 2)  # 点击指挥部
    ...
    if checkCount >= 20:
        return False
    time.sleep(0.2)
    mouseClick(SUPPLY_STEP1_CLICK_BOX, 1, 1)  # 点击补给
    mouseClick(AIRPORT_CLICK_BOX, 1, 1)
...
def planMode():
    print("ACTION: 计划模式")
    mouseClick(PLAN_MODE_CLICK_BOX, 1, 1)
...
def endCombat():
    print("ACTION: 战役结算")
    mouseClick(ROUND_END_CLICK_BOX, 5, 6)
    checkCount = 0
    while not is12_4e() and checkCount < 150:
